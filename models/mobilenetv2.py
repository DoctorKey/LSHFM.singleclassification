...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, T,
        ...
        # building classifier
        raw_feat_dim = self.last_channel
        if force_2FC or (student_dim != raw_feat_dim and student_dim != 0):
            ...
            self.classifier = nn.Linear(student_dim, feature_dim)
            print("=> 2FC: ({})->({})->({})".format(raw_feat_dim, student_dim, feature_dim))
        else:
            self.feat_fc = None
            self.classifier = nn.Linear(raw_feat_dim, feature_dim)
    ...
    def get_classifier_weight(self):
        return self.classifier.weight.data, self.classifier.bias.data
